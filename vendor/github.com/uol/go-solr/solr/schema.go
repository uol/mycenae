...
func (s *Schema) Post(path string, data interface{}) (*SolrUpdateResponse, error) {
	...
	if s.core != "" {
		r, err = HTTPPost(fmt.Sprintf("%s/%s/schema%s?wt=json", s.url.String(), s.core, newPath), b, [][]string{{"Content-Type", "application/json"}}, s.username, s.password)
	} else {
		r, err = HTTPPost(fmt.Sprintf("%s/schema%s?wt=json", s.url.String(), newPath), b, [][]string{{"Content-Type", "application/json"}}, s.username, s.password)
	}
	if err != nil {
		return nil, err
	}
