...
func (s *Schema) Get(path string, params *url.Values) (*SolrResponse, error) {
	...
	if s.core != "" {
		r, err = HTTPGet(s.client, fmt.Sprintf("%s/%s/schema%s?%s", s.url.String(), s.core, path, params.Encode()), nil, s.username, s.password)
	} else {
		r, err = HTTPGet(s.client, fmt.Sprintf("%s/schema%s?%s", s.url.String(), path, params.Encode()), nil, s.username, s.password)
	}
	if err != nil {
		return nil, err
	}
...
func (s *Schema) Post(path string, data interface{}) (*SolrUpdateResponse, error) {
	...
	if s.core != "" {
		r, err = HTTPPost(s.client, fmt.Sprintf("%s/%s/schema/%s?wt=json", s.url.String(), s.core, strings.Trim(path, "/")), b, [][]string{{"Content-Type", "application/json"}}, s.username, s.password)
	} else {
		r, err = HTTPPost(s.client, fmt.Sprintf("%s/schema/%s?wt=json", s.url.String(), strings.Trim(path, "/")), b, [][]string{{"Content-Type", "application/json"}}, s.username, s.password)
	}
	if err != nil {
		return nil, err
	}
